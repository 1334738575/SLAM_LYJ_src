...
namespace SLAM_LYJ_src
{
    ...
    SLAM_LYJ_API void testKdTree() {
        ...
        COMMON_LYJ::KdTree<double, 3> kdtree;
        auto funcDist = [](const COMMON_LYJ::KdTree<double, 3>::Node* _node, const Eigen::Vector3d& _p)->double {
    ...
    SLAM_LYJ_API void testPoint() {
        COMMON_LYJ::Point2d p2d(1, 3);
        std::cout << p2d[0] << std::endl;
        std::cout << p2d << std::endl;
        COMMON_LYJ::Point2d p2d2(3, 3);
        ...
        p2d[0] = 3;
        std::cout << p2d.x() << std::endl;
        COMMON_LYJ::Point2d p2d3 = p2d + p2d2;
        COMMON_LYJ::Point2d xx = 2 * p2d * 2;
        std::cout << "xx: " << xx << std::endl;
        //COMMON_LYJ::Point2i p2i(1, 1);
        //std::cout << (COMMON_LYJ::Point2i)(++p2i) << std::endl;
    }
    SLAM_LYJ_API void testArchive() {
        COMMON_LYJ::Point2d p2d(3, 3);
        COMMON_LYJ::Point2d p2d2;
        //std::stringstream ss;
    ...
    SLAM_LYJ_API void testTensor() {
        ...
        COMMON_LYJ::QRLYJ<4, 3> qr;
        qr.QR(mat.data(), Q.data(), R.data());
    }
    SLAM_LYJ_API void testCommonAlgorithm() {
        //Rodrigues
        auto R = COMMON_LYJ::Rodrigues2RotMatrix<double>(Eigen::Vector3d(1, 0, 0), 0.523);
        std::cout << R << std::endl;
        //�׳�
        uint64_t res1 = COMMON_LYJ::factorial(10, 9);
        std::cout << "�׳ˣ�" << res1 << std::endl;
        //����
        uint64_t res2 = COMMON_LYJ::pernutateNum(10, 2);
        std::cout << "����:" << res2 << std::endl;
        //���
        uint64_t res3 = COMMON_LYJ::selectNum(2, 1);
        std::cout << "���:" << res3 << std::endl;
        std::vector<std::vector<int>> results;
        COMMON_LYJ::fullSelection(10, 10, results);
    ...
    SLAM_LYJ_API void testRANSAC() {
        //ax+by+c=0
        std::vector<COMMON_LYJ::Point2d> points;
        int pSize = 10;
        ...
        int eveSize = 2;
        double errTh = 0.01;
        auto funcCalError = [&](const Eigen::Vector3d& _params, const COMMON_LYJ::Point2d& _p, double& _err)->bool {
        ...
        auto funcCalModule = [](const std::vector<const COMMON_LYJ::Point2d*>& _samples, Eigen::Vector3d& _params)->bool {
        ...
        COMMON_LYJ::RANSAC<COMMON_LYJ::Point2d, double, Eigen::Vector3d> ransac(inlineRatioTh, eveSize);
    ...
    SLAM_LYJ_API void testLine() {
        COMMON_LYJ::Line3d line3d(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, -1));
        COMMON_LYJ::Line3d line3d2(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(1, 0, 0));
        std::cout << COMMON_LYJ::Line3d::angleL2L(line3d, line3d2) << std::endl;
        std::cout << line3d.distP2L(Eigen::Vector3d(1, 0, 0)) << std::endl;
        return;
        COMMON_LYJ::Line2d line2d(Eigen::Vector2d(1, 0), Eigen::Vector2d(1, -1.71));
        std::cout << line2d << std::endl;
        std::cout << line2d.angle() << std::endl;
        std::cout << line2d.dist() << std::endl;
        COMMON_LYJ::Line2d line2d2(Eigen::Vector2d(0, 0), Eigen::Vector2d(1, 1));
        std::cout << line2d2 << std::endl;
        Eigen::Vector2d interP;
        if (COMMON_LYJ::Line2d::interestL2L(line2d, line2d2, interP))
        ...
        std::cout << COMMON_LYJ::Line2d::angleL2L(line2d, line2d2) << std::endl;
        cv::Mat m(100, 100, CV_8UC1, cv::Scalar(0));
        std::vector<Eigen::Vector2i> ps;
        COMMON_LYJ::Line2<int>::bresenhamLine(0, 0, 90, 50, ps);
    ...
    SLAM_LYJ_API void testPlane() {
        COMMON_LYJ::Plane3d plane3d(Eigen::Vector3d(0, 0, 1), Eigen::Vector3d(0, 0, 1));
        std::cout << plane3d << std::endl;
    }
    SLAM_LYJ_API void testPose() {
        COMMON_LYJ::Pose2D pose2D(1.57, Eigen::Vector2d(0, 0));
        std::cout << pose2D.getR() << std::endl;
    }
    SLAM_LYJ_API void testTriangler() {
        COMMON_LYJ::TrianglerPoint3DOption option;
        COMMON_LYJ::TrianglerPoint3D triangler;
    }
    SLAM_LYJ_API void testBitFlagVec() {
        COMMON_LYJ::BitFlagVec bfv(10);
        bfv.setFlag(5, true);
    ...
    SLAM_LYJ_API void testIncrementalAgvAndVar() {
        ...
        for (int i = 0; i < (int)v.size(); ++i) {
            COMMON_LYJ::calculateAgvAndVar<double, 1>(i, &v[i], &agv, &var);
    ...
    SLAM_LYJ_API void testGrid() {
        ...
        //COMMON_LYJ::Grid<double, 2> grid2Dd(10, points);
        COMMON_LYJ::Grid<double, 2> grid2Dd(10, std::vector<int>{100, 100}, Eigen::Vector2d(50, 50), points);
    ...
    SLAM_LYJ_API void testBuffer() {
        {
            ...
            COMMON_LYJ::Pose3D* poseCache = buffer.alloc<COMMON_LYJ::Pose3D>("pose");
            if (poseCache)
                std::cout << *poseCache << std::endl;
            COMMON_LYJ::Pose2D* poseCache2 = buffer.get<COMMON_LYJ::Pose2D>("pose2");
            if (!poseCache2)
                std::cout << "no such cache in buffer." << std::endl;
            COMMON_LYJ::Pose2D* poseCache3 = buffer.alloc<COMMON_LYJ::Pose2D>("pose");
            if (!poseCache3)
    ...
    SLAM_LYJ_API void testPatchMatch()
    {
        ...
        COMMON_LYJ::Grid2Df grid(100, h, w, kps);
    ...
    {
        COMMON_LYJ::Diffuser2D diffuser(6, 8, std::vector<Eigen::Vector2i>(), Eigen::Vector2i(3, 3));
    ...
    SLAM_LYJ_API void testPolarGrid()
    {
        ...
        COMMON_LYJ::MultiPolarGrid<int> multiPolarGrid(rs, 0.1f, 0, PI / 2, PI, 1);
        {
            ...
            for (uint32_t i = 0; i < sss; ++i) {
                if (multiPolarGrid.getCoordInGridById(i, loc)) {
                    ...
                    if (i == 1000 && false) {
                        ...
                        COMMON_LYJ::Pose3D pose(rot.cast<double>(), trans.cast<double>());
                        //Pose3D invPose = pose.inversed();
                        COMMON_LYJ::Pose3D invPose = pose;
        ...
        COMMON_LYJ::SoloPolarGrid<int> soloPolarGrid(2.0f, 0.1f, 0.1f, 0.1f, 0, PI / 2, PI);
        {
    ...
    {
        COMMON_LYJ::OcTreed tree1;
        COMMON_LYJ::OcTreef tree2;
        COMMON_LYJ::QuadTreed tree3;
        COMMON_LYJ::QuadTreef tree4;
        return;
    }
    ...
    SLAM_LYJ_API void testThreadPool() {
        COMMON_LYJ::ThreadPool threadPool(5);
        for (int i = 0; i < 100; ++i)
        {
